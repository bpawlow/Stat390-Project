...
write_csv(train, 'data/train3.csv')

folds <- vfold_cv(train, repeats = 4, v = 5)

write_rds(folds, 'model selection/model_objects/folds3.rds')
...
pca_train <- cbind(train, pca_obj %>% select(PC1,PC2,PC3))


write_csv(pca_train, 'data/pca_train3.csv')

folds_pca <- vfold_cv(pca_train, repeats = 4, v = 5)

write_rds(folds_pca,'model selection/model_objects/folds_pca3.rds')



test <-
  read_csv("data/test.csv") %>%  as_tibble() %>% select(id, all_of(fv)) %>% mutate(x516 = as_factor(x516))




pca_vars_test <- read_csv("data/test.csv") %>% as_tibble() %>% select(-all_of(fv)) %>% mutate(y = 1:n())

pca_vars_test <- pca_vars_test[!duplicated(as.list(pca_vars_test))]
...
pca_data_test <- recipe %>% prep() %>% bake(pca_vars_test)


test_pca <- pca_data_test %>% 
  select(-y) %>%
  scale() %>% # Standardize the variables
  prcomp() 
...
pca_obj_test<-as_tibble(test_pca$x)

pca_test <- cbind(test, pca_obj_test %>% select(PC1,PC2,PC3))


write_csv(pca_test, 'data/pca_test3.csv')



# Loading data ----
train <- read_csv('data/train3.csv')%>% mutate(x516 = as_factor(x516))
pca_train <- read_csv('data/pca_train3.csv') %>% mutate(x516 = as_factor(x516))
folds_pca <- read_rds( 'model selection/model_objects/folds_pca3.rds')
folds <- read_rds( 'model selection/model_objects/folds3.rds')
...
write_rds(bt_tuned, "model selection/model_objects/bt_tuned3.rds")
...
write_rds(bt_tuned_pca, "model selection/model_objects/bt_tuned_pca3.rds")
...
write_rds(nn_tuned, "model selection/model_objects/nn_tuned3.rds")
...
write_rds(nn_tuned_pca, "model selection/model_objects/nn_tuned_pca3.rds")
...
write_rds(cube_tuned, "model selection/model_objects/cube_tuned3.rds")
...
write_rds(cube_tuned_pca, "model selection/model_objects/cube_tuned_pca3.rds")
...
write_rds(svmpoly_tuned, "model selection/model_objects/svmpoly_tuned3.rds")
...
write_rds(svmpoly_tuned_pca, "model selection/model_objects/svmpoly_tuned_pca3.rds")
...
write_rds(bmlp_tuned, "model selection/model_objects/bmlp_tuned3.rds")
...
write_rds(bmlp_tuned_pca, "model selection/model_objects/bmlp_tuned_pca3.rds")
...
write_rds(svm_tuned, "model selection/model_objects/svm_tuned3.rds")
...
write_rds(svm_tuned_pca, "model selection/model_objects/svm_tuned_pca3.rds")
...
write_rds(knn_tuned, "model selection/model_objects/knn_tuned3.rds")
...
write_rds(knn_tuned_pca, "model selection/model_objects/knn_tuned_pca3.rds")
...
write_rds(rf_tuned, "model selection/model_objects/rf_tuned3.rds")
...
write_rds(rf_tuned_pca, 'model selection/model_objects/rf_tuned_pca3.rds')
...
write_rds(en_tuned, "model selection/model_objects/en_tuned3.rds")
...
write_rds(en_tuned_pca, "model selection/model_objects/en_tuned_pca3.rds")
...
write_rds(bag_tree_tuned, "model selection/model_objects/bag_tree_tuned3.rds")
...
write_rds(bag_tree_tuned_pca, "model selection/model_objects/bag_tree_tuned_pca3.rds")
...
write_rds(bag_mars_tuned, "model selection/model_objects/bag_mars_tuned3.rds")
...
write_rds(bag_mars_tuned_pca, "model selection/model_objects/bag_mars_tuned_pca3.rds")
...
write_rds(mars_tuned, "model selection/model_objects/mars_tuned3.rds")
...
write_rds(mars_tuned_pca, "model selection/model_objects/mars_tuned_pca3.rds")
